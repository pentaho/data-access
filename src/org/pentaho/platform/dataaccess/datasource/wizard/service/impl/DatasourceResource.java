...
public class DatasourceResource extends DataSourceWizardResource {
  ...
  public Response putMondrianSchema(
    ...
    try {
      ...
      service
        .putMondrianSchema( dataInputStream, schemaFileInfo, catalogName, origCatalogName, datasourceName, overwrite,
  ...
  public Response importMetadataDatasourceWithPost( @FormDataParam( "domainId" ) String domainId,
    ...
    throws PentahoAccessControlException {
    Response response = importMetadataDatasource( domainId, metadataFile, metadataFileInfo, overwrite, localeFiles, localeFilesInfo );
    ResponseBuilder responseBuilder;
    responseBuilder = Response.ok();
    responseBuilder.entity( String.valueOf( response.getStatus() ) );
    responseBuilder.status( 200 );
  ...
  public Response testConnection( DatabaseConnection connection ) throws ConnectionServiceException {
    ...
    if ( success ) {
      ...
      return Response.serverError().entity( Messages.getErrorString( "ConnectionServiceImpl.ERROR_0009_CONNECTION_FAILED", connection.getDatabaseName() ) ).build();
    }
  }
