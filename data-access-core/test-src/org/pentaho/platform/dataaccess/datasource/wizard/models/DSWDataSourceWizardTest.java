...
public class DSWDataSourceWizardTest {
  ...
  public void testGetTemplates() throws Exception {
    ...
    for (int i=0; i < templateDefinition.length; i++) {
      ...
      for (IDSWTemplate template : templateList){
        if (templateDefinition[i][0].equals(template.getID()) && templateDefinition[i][1].equals(template.getDisplayName(Locale.getDefault()))){
          if (found == true) {
  ...
  private String[][] setupMockTemplateList() {
    final String[][] templateDefinition = new String[][] {{"dummy1", "dummy1Name"} , {"dummy2", "dummy2Name"}, {"SQL", "name_gets_replaced"}} ;
