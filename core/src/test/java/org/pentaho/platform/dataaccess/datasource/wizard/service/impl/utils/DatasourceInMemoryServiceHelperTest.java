...
public class DatasourceInMemoryServiceHelperTest {
  ...
  public void setUp() throws Exception {
    ...
    when( resLoader.getPluginSetting( Mockito.<ClassLoader>any(), anyString(), anyString() ) )
      .thenReturn( SimpleDataAccessPermissionHandler.class.getName() );
    when( policy.isAllowed( anyString() ) ).thenReturn( true );
    pentahoObjectFactory = mock( IPentahoObjectFactory.class );
    when( pentahoObjectFactory.objectDefined( anyString() ) ).thenReturn( true );
    when( pentahoObjectFactory.get( any(), any(), Mockito.<IPentahoSession>any() ) ).thenAnswer(
        new Answer<Object>() {
          @Override
  ...
  public void testGetCsvDataSample_withtHeader() {
    testGetCsvDataSample_Base( true, "1", 5, ";", csvFile );
  }

  @Test
  public void testGetCsvDataSample_withoutHeader() {
    testGetCsvDataSample_Base( false, "id", 3, ";", csvFile );
  }

  @Test
  public void testGetCsvDataSample_differentDelimetr() {
    testGetCsvDataSample_Base( false, "id;name;", 3, ":", csvFile );
  }
